module.exports = function(App, helpenName='modifiers'){ 

  const mValidAgentTypesExp = new RegExp('mozilla|chrome|opera|safari|windows|macintosh|x11|linux|gecko|yabrowser|edg|yowser','gi');

  return {
    noBots: async(req, res, next)=>{
      try{
        const _input = (req.headers['user-agent']||'').toLowerCase();
        if( !_input.match(mValidAgentTypesExp) && !req.headers['cookie'] )
          return await App.json(res, 401, 'not authorized:[nbalw]');
        return next();
      }catch(e){
        console.error(` #${helpenName}.noBots: ${e.message} `);
        return next();
      }
    },
    isBot: async(req)=>{
      try{
        const _input = (req.headers['user-agent']||'').toLowerCase();
        return ( !_input.match(mValidAgentTypesExp) && !req.headers['cookie'] )
      }catch(e){
        console.error(` #${helpenName}.isBot: ${e.message} `);
        return false;
      }
    },
  };
...
// "Mozilla/5.0 (Linux; U; Android 4.3; en-us; SM-N900T Build/JSS15J) AppleWebKit/534.30 (KHTML, like Gecko|YaBrowser|Edg|Yowser) Version/4.0 Mobile Safari/534.30",
...
// "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/96.0.4664.110 YaBrowser/22.1.0.2517 YaBrowser|Edg|Yowser/2.5 Safari/537.36",
