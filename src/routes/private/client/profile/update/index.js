...
module.exports = function(App, RPath){
	  console.log('[PROFILE UPDATE] Router initialized with path:', RPath);
router.use((req, res, next) => {
    console.log(`[PROFILE UPDATE] Request received: ${req.method} ${req.originalUrl}`);
    next();
  });

	  router.use('/resend', async(req, res) => {
    try {
      console.log('[RESEND] Verification email requested');

      const mUser = await req.user;

      console.log(`[RESEND] User ID: ${mUser.id}, Email: ${mUser.email}, IsEmailVerified: ${mUser.isEmailVerified}`);
      ...
      console.log(`[RESEND] Created email verification record: ${mEmailVerification ? mEmailVerification.id : 'Failed'}`);

      if (!App.isObject(mEmailVerification) || !App.isPosNumber(mEmailVerification.id)) {
        console.error(`[RESEND] Failed to create email verification record`);
        return App.json(res, false, App.t(['failed', 'to', 'create', 'email-verification'], req.lang));
      }

      // Send the verification email
      console.log(`[RESEND] Preparing to send email to: ${mUser.email}`);
      ...
      console.log(`[RESEND] Email template created: ${emailTemplate ? 'Yes' : 'No'}`);
      ...
      console.log(`[RESEND] Email send result: ${JSON.stringify(sendRes)}`);

      if (!sendRes.success) {
        console.error(`[RESEND] Failed to send email: ${sendRes.message}`);
