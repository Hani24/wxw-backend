...
// /private/courier/orders/current/set-as-delivered
    ...
    try{
      ...
      try{
        ...
        const updateOrder = await mOrder.update({
          ...
          isClientDidGetInTouch: true,
          clientDidGetInTouchAt: App.getISODate(),
          checksum: true,
        }, {transaction: tx});
...

      {
        const pushToClientRes = await App.getModel('ClientNotification')
          .pushToClient( mOrder.Client, {
            type: App.getModel('ClientNotification').getTypes()['courierDeliveredOrder'],
            title: `Order #${mOrder.id} ${App.t(['has been delivered'], req.lang)}.`,
            message: App.t(['Enjoy your meal!'], req.lang),
            data: metadata,
          });
      ...
      if( false ){
        ...
        const notifyData = {
          ack: false,
          event: App.getModel('RestaurantNotification').getEvents()['courierDeliveredOrder'],
          type: App.getModel('RestaurantNotification').getTypes()['courierDeliveredOrder'],
          data: metadata, 
        };
