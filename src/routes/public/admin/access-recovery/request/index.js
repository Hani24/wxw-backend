...
// /public/admin/access-recovery/request/
...
module.exports = function(App, RPath){
  ...
  router.use('', App.modifiers.noBots, async(req, res)=>{
    ...
    try{
      ...
      const mUser = await App.getModel('User').findOne({where: {email}});
      ...
      if( App.isObject(mAccessRecovery) && App.isNumber(mAccessRecovery.id) ){
        ...
        const mailResentRes = await App.Mailer.send({
          ...
          data: await App.Mailer.createEmailTemplate('admin-access-recovery', { 
            lang: mUser.lang,
            code: mAccessRecovery.code,
            path: App.toAppPath( 'web', 'admin.access-recovery-verify', mAccessRecovery.code, false),
          })
        });
