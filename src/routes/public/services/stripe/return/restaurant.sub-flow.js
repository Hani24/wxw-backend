module.exports = async(App, req, res, {data, accountId, mRestaurant}={})=>{

  try{

    // KYC procedure successfully completed.
    const action = {
      name: 'Go to the Restaurant Dashboard',
      link: App.getEnv('UI_RESTAURANT_PROTODOMAIN'),
      type: 'action-link', // action-button
    };

    const mUser = await App.getModel('User').getByFields({ id: mRestaurant.userId });
    ...
    if( /*mUser.isVerified &&*/ mRestaurant.isKycCompleted ){
      return await App.renderUI( res, 'message', {
        ...
        message: App.t(['KYC procedure successfully completed.'], req.lang, ''),
    ...
    if( verification.status !== 'verified' ){
      return await App.renderUI( res, 'message', {
        header: App.t(['KYC Verification'], req.lang),
    ...
    const updateCourier = await mRestaurant.update({
      isKycCompleted: true,
